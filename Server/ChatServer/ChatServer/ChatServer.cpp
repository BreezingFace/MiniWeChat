...
int main()
{
    ...
    try {
        ...
        signals.async_wait([&io_context, pool,&server](auto, auto) {//const boost::system::error_code& ec, int signo
            io_context.stop();
            pool->Stop();
            server->Shutdown();
            });
        auto port_str = cfg["SelfServer"]["Port"];
        //auto port = static_cast<unsigned short>(atoi(port_str.c_str()));
