...
ConfigMgr::ConfigMgr()
{
	...
	// 构建config.ini文件的完整路径  
	boost::filesystem::path config_path = current_path / "config.ini";//path类重载了/运算符，用来进行路径连接
	...
	for (const auto& section_pair : pt) {
		...
		for (const auto& key_value_pair : section_tree) {
			...
			const std::string& value = key_value_pair.second.get_value<std::string>();//将ptree转为string
