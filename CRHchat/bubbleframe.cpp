...
{
    if(m_pHLayout->count() > 0) // 如果布局中已有控件，直接返回
        return ;
    else{
        m_pHLayout->addWidget(w);// 否则添加新控件
    }
}
...
void BubbleFrame::paintEvent(QPaintEvent *e)
{
    ...
    if(m_role == ChatRole::Other)
    {
        ...
        //画小三角
        QPointF points[3] = {
            QPointF(bk_rect.x(), 12),//8,12
            QPointF(bk_rect.x(), 10+WIDTH_SANJIAO +2),//8,20
            QPointF(bk_rect.x()-WIDTH_SANJIAO, 10+WIDTH_SANJIAO-WIDTH_SANJIAO/2),//0,14
        };
