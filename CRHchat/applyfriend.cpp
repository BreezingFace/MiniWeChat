    ...
    ui(new Ui::ApplyFriend),_label_point(2,6)
{
    ...
    this->setModal(true);
    ui->name_ed->setPlaceholderText(tr("你给对方的备注"));
    ui->lb_ed->setPlaceholderText("搜索、添加标签");
    ui->back_ed->setPlaceholderText("添加对方备注名");

    ui->lb_ed->SetMaxLength(21);//最多输入的标签数量
    ui->lb_ed->move(2, 2);
...
void ApplyFriend::addLabel(QString name)
{
    ...
    if (_label_point.x() + tmplabel->width() > max_width) {
        _label_point.setY(_label_point.y() + tmplabel->height() + 6);//换行
        _label_point.setX(2);
    }
    else {
    }
    tmplabel->move(_label_point);//移动到_label_point开始的位置
    tmplabel->show();
    ...
    void ApplyFriend::SlotLabelEnter()
{
    ...
    AddTipLbs(lb, _tip_cur_point, next_point, textWidth, textHeight);
    _tip_cur_point = next_point;
    //tip-offset是lb_list的内边距
    int diff_height = next_point.y() + textHeight + tip_offset - ui->lb_list->height();//diff_height>=0,因为ui->lb_list->height()的计算（即setFixedHeight）滞后于next_point.y() + textHeight + tip_offset
    ui->lb_list->setFixedHeight(next_point.y() + textHeight + tip_offset);
...
void ApplyFriend::SlotRemoveFriendLabel(QString name)
{
    ...
    _label_point.setX(2);
    _label_point.setY(6);//重新初始化x,y坐标，因为后续需要从头开始  resetLabels();
    ...
    if(find_key != _friend_label_keys.end()){
        _friend_label_keys.erase(find_key);//删除对应的标签
    }
    delete find_iter.value();//去掉对应的值
    _friend_labels.erase(find_iter);//去掉对应的键
    resetLabels();
    auto find_add = _add_labels.find(name);
    if(find_add == _add_labels.end()){
        return;
    }
    find_add.value()->ResetNormalState();//这里并没有把lb_list的标签删除，而是把置为normal状态，即显示灰色
}
...
void ApplyFriend::SlotApplySure()
{
    ...
    QByteArray jsonData = doc.toJson(QJsonDocument::Compact);//选择压缩方式为compact
