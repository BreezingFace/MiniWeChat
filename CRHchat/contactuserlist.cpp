...
/*
 * addItem(item)
将 QListWidgetItem 添加到 ContactUserList（即 this）中，此时 item 成为列表的一部分。
setItemWidget(item, widget)
将自定义的 widget（如 GroupTipItem）绑定到 item 上，实现视觉呈现。
关系：item 是数据的载体，widget 是数据的可视化。
setSizeHint()
通过 item->setSizeHint(widget->sizeHint()) 确保 item 的大小与自定义 widget 匹配。
setFlags()
控制 item 的交互行为（如禁止选中 ~Qt::ItemIsSelectable）。

QListWidgetItem
负责项的底层管理（位置、状态、数据），但无界面。
自定义 QListWidget（ContactUserList）
作为容器，管理所有 QListWidgetItem 的布局和交互。
自定义 QWidget（如 ConUserItem）
通过 setItemWidget() 绑定到 QListWidgetItem，实现个性化显示。
这种设计分离了 数据管理（QListWidgetItem）和 界面显示（自定义 QWidget），提供了灵活性。
 */
...
bool ContactUserList::eventFilter(QObject *watched, QEvent *event)
{
    ...
    if (watched == this->viewport() && event->type() == QEvent::Wheel) {
        ...
        if (maxScrollValue - currentValue <= 0) {
            ...
            QTimer::singleShot(100, [this](){//延时100ms将_load_pending置为false
                _load_pending = false;
...
//验证申请好友方是否已经是好友，如果是则直接返回，否则添加好友
