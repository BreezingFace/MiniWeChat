...
{
    ui->setupUi(this);
    connect(ui->reg_btn,&QPushButton::clicked,this,&LoginDialog::switchRegister); //鼠标点击触发reg_btn，clicked函数响应，并通知给本对象，并进一步触发switchRegister函数
    ...
    //连接tcp管理者发出的连接成功信号
    connect(TcpMgr::GetInstance().get(),&TcpMgr::sig_connect_success,this,&LoginDialog::slot_tcp_con_finish);
    //练剑tcp管理者发出的连接失败信号
...
void LoginDialog::slot_login_mod_finish(ReqId id, QString res, ErrorCodes err)
{
    ...
    QJsonDocument jsonDoc = QJsonDocument::fromJson(res.toUtf8());//先将QString转为QByteArray，在转为QJsonDocument
    //json解析错误
    if(jsonDoc.isNull()){
...
void LoginDialog::slot_tcp_con_finish(bool bsuccess)
{
    if(bsuccess){
       ...
       // 指定输出的格式化风格为 带缩进的排版（美观易读）
       // 生成的字符串会包含换行和空格（默认缩进4个空格）
        //涉及到网络传输，用QByteArray而非QString
        /*
        需求场景	推荐类型
        直接显示/编辑 JSON 文本	QString
        网络传输、文件读写	QByteArray
        需要调用文本处理函数（如正则）	QString
        加密/压缩/校验操作	QByteArray
            */
