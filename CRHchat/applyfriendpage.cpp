...
void ApplyFriendPage::AddNewApply(std::shared_ptr<AddFriendApply> apply)
{
    ...
    auto apply_info = std::make_shared<ApplyInfo>(apply->_from_uid,
                                                  apply->_name, apply->_desc,heads[head_i], apply->_nick, 0, 0);
    ...
    _unauth_items[uid] = apply_item;// 存储未处理的申请项
...
void ApplyFriendPage::loadApplyList()
{
    ...
    for(auto &apply: apply_list){
        ...
        item->setSizeHint(apply_item->sizeHint());
        item->setFlags(item->flags() & ~Qt::ItemIsEnabled & ~Qt::ItemIsSelectable);//禁用交互（防止用户选中）

         // 插入到列表顶部（insertItem(0, item)）
        ui->apply_friend_list->insertItem(0,item);
        ui->apply_friend_list->setItemWidget(item, apply_item);
    ...
    for(int i = 0; i < 13; i++){
        ...
        auto apply = std::make_shared<ApplyInfo>(0, names[name_i], strs[str_i],
                                                 heads[head_i], names[name_i], 0, 1);//最后的1表示已添加
        apply_item->SetInfo(apply);
        QListWidgetItem *item = new QListWidgetItem;
        //qDebug()<<"chat_user_wid sizeHint is " << chat_user_wid->sizeHint();
        item->setSizeHint(apply_item->sizeHint());
...
void ApplyFriendPage::slot_auth_rsp(std::shared_ptr<AuthRsp> auth_rsp)
{
    ...
    find_iter->second->ShowAddBtn(false);//已添加好友，就无需再次添加了
}
