...
{
    mGetMsgList.clear();  // 清空临时消息列表(vector)
    QString doc = this->document()->toPlainText();  // 获取纯文本内容
    QString text = "";   // 临时存储连续文本
    int indexUrl = 0;    // 遍历 mMsgList 的索引
    int count = mMsgList.size();  // 原始消息列表大小

    for(int index=0; index<doc.size(); index++)
    {
        if(doc[index]==QChar::ObjectReplacementCharacter)//QChar::ObjectReplacementCharacter：Qt 中表示富文本内嵌对象（如图片、表格等）的占位符。当遇到此字符时，说明当前位置是一个嵌入对象。
        {
            /*
             * 如果之前有未处理的文本（text 非空），先将其作为一条文本消息保存。
            遍历 mMsgList（原始消息列表），找到与当前对象匹配的 MsgInfo（通过对比 HTML 内容）。
            将匹配到的对象消息添加到 mGetMsgList。
             */
            if(!text.isEmpty())
            {
                QPixmap pix;
        ...
        //若当前字符不是对象占位符，则将其追加到临时字符串 text 中
        else
        {
...
void MessageTextEdit::insertImages(const QString &url)
{
    ...
    //按比例缩放图片
    /*
     * 作用：如果图片宽度 > 120 或高度 > 80，则按比例缩放。
        逻辑:
        宽度优先：如果图片宽度 > 高度，缩放到宽度 120，高度按比例调整。
        高度优先：如果图片高度 > 宽度，缩放到高度 80，宽度按比例调整。
     */
